...
export default function GlobalSearch() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.8)", display: "flex", justifyContent: "center", paddingTop: "10vh" }} onClick={() => setIsOpen(false)}>
          <div style={{ width: "100%", maxWidth: "600px", background: "var(--bg-card)", borderRadius: "16px", border: "1px solid var(--border)", overflow: "hidden", boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", alignItems: "center", padding: "1rem", borderBottom: "1px solid var(--border)" }}>
              ...
              />
              <button onClick={() => setIsOpen(false)} style={{ background: "none", border: "none", color: "#8896B3", cursor: "pointer" }}>
                <X size={20} />
              </button>
            </div>
            ...
            <div style={{ maxHeight: "400px", overflowY: "auto", padding: "0.5rem" }}>
              {results.length > 0 ? (
                results.map((item, i) => (
                  <button key={i} onClick={() => handleSelect(item.href)} style={{ width: "100%", display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem 1rem", border: "none", background: "transparent", borderRadius: "8px", cursor: "pointer", textAlign: "left" }} className="search-item">
